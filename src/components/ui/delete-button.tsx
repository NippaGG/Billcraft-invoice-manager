...
const FOCUS = "outline-none focus-visible:ring-2 focus-visible:ring-red-500/60";
const ACCENT = "#EF4444";
  ...
  return (
    <motion.div className="flex" variants={reduced ? undefined : circleMotion}>
      <motion.button
        ...
        transition={PRESS}
        className={cn(
          "grid place-items-center rounded-full transition-colors duration-200 hover:bg-[#FAFAFD] dark:hover:bg-[#2C2C2C] cursor-pointer",
          circleSizeClass,
          FOCUS,
          SURFACE,
          LIFT
...
export function DeleteButton({
  ...
  const TILE = isSmall ? 36 : 48;
  const PANEL = isSmall ? 68 : 84;
  ...
  return (
    <motion.div
      ...
      data-status={status}
      className={cn(
        "relative rounded-xl select-none shrink-0 overflow-hidden",
        isSmall ? "h-9 rounded-xl" : "h-12 rounded-2xl",
        SURFACE,
        GLYPH,
        className
      )}
    ...
    >
      <motion.button
        ...
        transition={TAP}
        className={cn(
          "relative z-10 grid place-items-center cursor-pointer",
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ width: PANEL }}
            className={cn(
              "absolute inset-y-0 right-0 flex items-center justify-center gap-1.5 rounded-xl",
              isSmall ? "gap-1 rounded-xl pr-1.5" : "gap-2 rounded-2xl",
              RECESS
            )}
