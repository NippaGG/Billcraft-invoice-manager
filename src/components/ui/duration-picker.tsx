    ...
    return (
        <>
            <motion.input
                ...
                className='h-full text-center font-bold text-foreground outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none bg-transparent'
            />
            <span ref={measureRef} aria-hidden className="invisible absolute whitespace-pre font-bold">{value || '0'}</span>
        </>
    )
}
...
function DurationPicker({
    ...
    return (
        <motion.div
            ...
            className={cn('inline-flex flex-row items-center justify-center text-foreground select-none', disabled && 'opacity-50 pointer-events-none', className)}
            {...props}
        >
            <SquircleSegment leftRadius={CORNER_RADIUS} rightRadius={innerRadius} style={{ paddingRight: innerPadRight }} className="bg-foreground/[0.05] dark:bg-white/[0.07] border border-card-border h-10 flex items-center gap-1 pl-2">
                ...
                <motion.span style={{ x: swayX }} className='text-muted text-xs font-semibold pr-1.5'>{hoursLabel}</motion.span>
            </SquircleSegment>
            <SquircleSegment leftRadius={innerRadius} rightRadius={innerRadius} style={{ marginLeft: segmentSpacing, paddingLeft: innerPadLeft, paddingRight: innerPadRight }} className="bg-foreground/[0.05] dark:bg-white/[0.07] border border-card-border h-10 flex items-center gap-1">
                ...
                <motion.span style={{ x: swayX }} className='text-muted text-xs font-semibold pr-1.5'>{minutesLabel}</motion.span>
            </SquircleSegment>
            <SquircleSegment asChild leftRadius={innerRadius} rightRadius={CORNER_RADIUS} style={{ marginLeft: segmentSpacing }}>
                <button
                    ...
                    className='w-10 h-10 bg-foreground/[0.05] dark:bg-white/[0.07] border border-card-border flex justify-center items-center active:scale-90 transition-transform disabled:active:scale-100 cursor-pointer hover:bg-foreground/[0.08] dark:hover:bg-white/[0.12]'
                >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16">
                        <motion.path fill="currentColor" stroke="currentColor" strokeWidth={0} strokeLinejoin="round" strokeLinecap="round" style={{ strokeWidth: iconStrokeWidth, strokeOpacity: iconStrokeOpacity }} d={iconPath} className="text-muted" />
                        <motion.path d="M14 6L18 10" fill="none" strokeWidth={1.5} strokeLinecap="round" className="stroke-foreground/10" style={{ opacity: iconDashOpacity }} />
                    </svg>
                </button>
            </SquircleSegment>
        </motion.div>
...
export function parseDurationString(str?: string): DurationValue {
    ...
    if (!hoursMatch && !minutesMatch && !isNaN(Number(str))) {
        const n = Number(str)
        return { hours: Math.floor(n), minutes: Math.round((n % 1) * 60) }
    }
    return { hours, minutes }
}

export function formatDurationString(val: DurationValue): string {
    const parts: string[] = []
    if (val.hours > 0) parts.push(`${val.hours}h`)
    if (val.minutes > 0) parts.push(`${val.minutes}m`)
